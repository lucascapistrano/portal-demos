...
public class Initializer {
	...
	private final Map<String, App> apps = new LinkedHashMap<String, App>();

	private final List<Preparer> preparers = new ArrayList<Preparer>();

	public Initializer init(String... packages) throws IOException {
	...
	private App createApp(String name) {
		...
		try {
			...
			socketManager = (SocketManager) Class.forName(
					"com.github.flowersinthesand.portal.atmosphere.AtmosphereSocketManager").newInstance();
		} catch (Exception e) {
			logger.error("", e);
		}
		...
		return app.set(App.NAME, name).set(App.EVENT_DISPATCHER, new DefaultEventDispatcher())
